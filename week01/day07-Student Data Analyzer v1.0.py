...
def format_major_count(students):
    """
    Helper function: showing the number of each major.
    students: list[Student]
    """
    count = count_majors(students)
    message = ""
    message += "Major Distribution:\n"
    for major, number in count.items():
        message += f"{major}: {number}\n"
    return message
...
def student_report(students):
    ...
    message += format_major_count(students)
    message += "\n\n"
